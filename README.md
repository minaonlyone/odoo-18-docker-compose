...
Install [docker](https://docs.docker.com/get-docker/) and [docker-compose](https://docs.docker.com/compose/install/) yourself, then run the following to set up first Odoo instance @ `localhost:10018` (default master password: `smartsupport.tech`):

``` bash
curl -s https://raw.githubusercontent.com/minaonlyone/odoo-18-docker-compose/master/run.sh | bash -s odoo-one 10018 20018
```
and/or run the following to set up another Odoo instance @ `localhost:11018` (default master password: `smartsupport.tech`):

``` bash
curl -s https://raw.githubusercontent.com/minaonlyone/odoo-18-docker-compose/master/run.sh | bash -s odoo-two 11018 21018
```
...
* Default database password (**admin_passwd**) is `smartsupport.tech`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
...
This project is maintained by **Mina Adel - SmartSupport**.

For support and inquiries, please contact us at smartsupport.tech
